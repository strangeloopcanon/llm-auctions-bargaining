...
def aggregate(rows: List[Dict[str, Any]]) -> List[Tuple[str, str, str, Dict[str, float]]]:
    grouped: Dict[Tuple[str, str, str], List[Dict[str, Any]]] = defaultdict(list)
    for row in rows:
        grouped[(row.get("scenario", ""), row.get("model", ""), row.get("arm", ""))].append(row)

    results = []
    for (scenario, model, arm), items in sorted(grouped.items()):
        ...
        metrics = {
            ...
            "partner_request_rate": mean(
                summary.get("partner_request_rate", 0.0) for summary in summaries
            ),
            "partner_requests_submitted": mean(
                summary.get("partner_requests_submitted", 0.0) for summary in summaries
            ),
        ...
        results.append((scenario, model, arm, metrics))
    return results


def print_markdown_table(aggregates: List[Tuple[str, str, str, Dict[str, float]]]) -> None:
    print(
        "| scenario | model | arm | runs | fulfillment_rate | started_not_delivered | "
        "partner_request_rate | partner_requests_submitted | deal_volume | "
        "substitute_builds_started | substitute_builds_completed | welfare | "
        "customer_value_captured |"
    )
    print(
        "| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |"
    )
    for scenario, model, arm, metrics in aggregates:
        print(
            "| "
            f"{scenario} | {model} | {arm} | {int(metrics['runs'])} | "
            f"{metrics['fulfillment_rate']:.3f} | {metrics['started_not_delivered_count']:.2f} | "
            f"{metrics['partner_request_rate']:.3f} | {metrics['partner_requests_submitted']:.2f} | "
