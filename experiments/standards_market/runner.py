...
from .generator import DEFAULT_DEADLINE_ROUND, DEFAULT_ROUNDS, generate_firms, generate_private_costs, generate_project
from .market import run_standards_market


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the standards initiative benchmark.")
    ...
    parser.add_argument("--output-dir", type=str, default="runs_standards_market")
...
def main() -> None:
    ...
    output_path = out_dir / "standards_market_results.jsonl"
    ...
    for offset in range(args.seeds):
        ...
        project = generate_project(seed=seed, deadline_round=args.deadline_round)
        private_costs = generate_private_costs(seed=seed)
        for model in models:
            for arm in arms:
                ...
                row = {
                    ...
                    "firms": result["firms"],
                    "build_records": result["build_records"],
                    "public_memos": result["public_memos"],
                }
                rows.append(row)
                write_jsonl(output_path, rows)
                print(
                    f"Completed standards market seed={seed} arm={arm} model={model} "
